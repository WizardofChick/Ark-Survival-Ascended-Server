...
- Staged ARK downloads use `ServerFiles/arkserver/.pok-manager/update/staging` rather than the temporary container's writable `/tmp` layer. The manager checks that shared filesystem for at least 25,360 MB of free space before SteamCMD starts and removes the per-run staging directory afterward; an instance-specific `/tmp` mount is not required for manual updates.
- Manual updates honor the Docker sudo preference saved during manager setup. A no-sudo installation never falls back to an interactive sudo prompt, including under cron; sudo-enabled unattended jobs require non-interactive sudo authorization.
...
Direct `API=FALSE` launches do not use Steam's game-specific ProtonFixes hook path. Complete launch diagnostics remain available at `/home/pok/logs/proton_runtime.log`; routine GameAnalytics telemetry is omitted from Docker output and automatic failure summaries, while other final diagnostic lines are shown if both pinned-Proton attempts fail.
